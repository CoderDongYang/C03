  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push("/experiments/new")}>+ 新建实验</Button>
      </div>

      <div className="flex items-center gap-2">
        <Switch checked={includeArchived} onCheckedChange={setIncludeArchived} />
        <Label className="cursor-pointer" onClick={() => setIncludeArchived(!includeArchived)}>显示已归档</Label>
      </div>
          ...
          {experiments.map((experiment) => (
            <Card key={experiment.id} className="flex flex-col">
              <CardHeader>
                ...
                <CardDescription>目标事件：{experiment.targetEvent}</CardDescription>
              </CardHeader>
              <CardContent className="flex-1">
                {experiment.description && (
                  <p className="text-sm text-muted-foreground mb-3 line-clamp-2">{experiment.description}</p>
                )}
                <p className="text-xs text-muted-foreground">创建于 {formatDate(experiment.createdAt)}</p>
                <p className="text-xs text-muted-foreground mt-1">{experiment.versions.length} 个版本</p>
              </CardContent>
              <CardFooter className="flex gap-2">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => router.push(`/experiments/${experiment.id}`)}>查看详情</Button>
                <Button variant="destructive" size="sm" onClick={() => setDeleteId(experiment.id)}>删除</Button>
              </CardFooter>
            </Card>
          ))}
        </div>
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>确定要删除这个实验吗？此操作不可撤销。</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteId(null)} disabled={deleteLoading}>取消</Button>
            <Button variant="destructive" onClick={handleDelete} disabled={deleteLoading}>{deleteLoading ? "删除中..." : "删除"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
